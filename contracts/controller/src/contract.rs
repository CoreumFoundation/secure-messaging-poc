...
use utils::{
    msg::{MessagesExecuteMsg, MessagesInstantiateMsg, ProfilesExecuteMsg, ProfilesInstantiateMsg},
    query::{ProfileInfo, ProfilesQueryMsg},
};

use crate::{
    error::ContractError,
    msg::{ContractAddressesResponse, ExecuteMsg, InstantiateMsg, QueryMsg},
...
pub fn instantiate(
    ...
    let wasm_profiles_msg = WasmMsg::Instantiate {
        ...
        msg: to_json_binary(&ProfilesInstantiateMsg {})?,
        funds: vec![],
    ...
    let wasm_messages_msg = WasmMsg::Instantiate {
        ...
        msg: to_json_binary(&MessagesInstantiateMsg {
...
fn create_profile(
    ...
    let msg = CosmosMsg::Wasm(WasmMsg::Execute {
        ...
        msg: to_json_binary(&create_profile_msg)?,
        funds: vec![],
    });
...
fn change_user_id(
    ...
    let msg = CosmosMsg::Wasm(WasmMsg::Execute {
        contract_addr: profile_address.to_string(),
        msg: to_json_binary(&change_userid_msg)?,
        funds: vec![],
    });
...
fn change_pubkey(
    ...
    let msg = CosmosMsg::Wasm(WasmMsg::Execute {
        contract_addr: profile_address.to_string(),
        msg: to_json_binary(&change_pubkey_msg)?,
        funds: vec![],
    });
...
fn send_message(
    ...
    let msg = CosmosMsg::Wasm(WasmMsg::Execute {
        ...
        msg: to_json_binary(&create_send_msg)?,
        funds: funds_to_send,
    });
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::ContractAddresses {} => to_json_binary(&query_contract_addresses(deps)?),
    }
}
...
fn query_contract_addresses(deps: Deps) -> StdResult<ContractAddressesResponse> {
    let profiles_contract_addr = PROFILES_ADDRESS.load(deps.storage)?;
    let messages_contract_addr = MESSAGES_ADDRESS.load(deps.storage)?;

    Ok(ContractAddressesResponse {
        profiles_contract_addr,
        messages_contract_addr,
    })
}
